...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if (f.ARMED)  {
    ...
    #if defined(POT_A)
      int ku = ((constrain(rcData[POT_A],1000,2000) -1000) << 1) >> 6 ; // * 20/640;
      //kp = ku*3 + 13; //range: 6.4-14.5
 ...
 //*********************************** 
  ...
  for(axis=0;axis<2;axis++) {
    ...
    if (f.ANGLE_MODE || f.HORIZON_MODE) { // axis relying on ACC
      ...
      errorAngle         = constrain(rc + GPS_angle[axis],-900,+900) - att.angle[axis] + conf.angleTrim[axis]; //16 bits is ok here
  ...
  for(axis=0;axis<3;axis++) {
    ...
    if ((f.ANGLE_MODE || f.HORIZON_MODE) && axis<2 ) { // MODE relying on ACC
      ...
      errorAngle = constrain((rcCommand[axis]<<1) + GPS_angle[axis],-900,+900) - att.angle[axis] + conf.angleTrim[axis]; //16 bits is ok here
    }
